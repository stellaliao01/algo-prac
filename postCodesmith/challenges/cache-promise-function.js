...
const memoize = (func) => {
  // declare cache variable and assign it to an empty object
  const cache = {};
  // create our return function that takes any JSON convertible value as input
  return function (input) {
    // convert the input to a JSON string and store it as inputStr
    const inputStr = JSON.stringify(input);
    // check if this inputStr key already exists in our cache
    // if so return the associated value
    if (cache[inputStr]) {
      return cache[inputStr];
    }
    // otherwise invoke the original function with our input as an argument and store the result as the value associated with our inputStr key
    cache[inputStr] = func(input);
    // return value stored at the inputStr key
    return cache[inputStr];
  };
};
...
const cachePromiseFunction = (get) => {
  // create an object to store results of get p-function as key value pairs
  const cache = {};

  return function (url) {
    if (cache.hasOwnProperty(url)) {
      // the data stored at memory[url] is not a promise so we need to use Promise.resolve, which returns a Promise object that is resolved with the given value (our data)
      return Promise.resolve(cache[url]); //this is a promise
    }

    // giving us a promise object because get returns a promise, .then also returns a promise
    return get(url).then((data) => {
      cache[url] = data;
      // console.log(data instanceof Promise) //this is not a promise
      return data; // this is not a promise
    });
  };
};
...
// let cachedFakeApi = cachePromiseFunction(fakeApi);
// console.log(cachedFakeApi('foo.com'));
...
const cachePromiseFunction2 = (get) => {
  ...
  return url => new Promise(resolve => {
    ...
    if (!pending[url]) {
      ...
      get(url).then(data => {
        cache[url] = data;
        pending[url].forEach(res => res(data));
        delete pending[url];
      });
    }
